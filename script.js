...
const ctx = canvas.getContext('2d'); //allows us to access a range of drawing functions so that we can manipulate what's on the canvas

const img = new Image();


const reader = new FileReader();  // allows us to read the contents of any uploaded file.
...
function greyscale() {
     const imageData = ctx.getImageData(0,0,canvas.width, canvas.height); // 0 to full width , 0 to full height
...
function download() {
    const image = canvas.toDataURL();8
...
imageLoader.addEventListener('change', uploadImage);    //study eventListeners!!